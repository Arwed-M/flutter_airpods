...
class AirPodsStreamHandler: NSObject, FlutterStreamHandler, CMHeadphoneMotionManagerDelegate {
    ...
    func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        if #available(macOS 14.0, *) {
            ...
            // For older macOS versions, send an error or empty data
            events(FlutterError(code: "UNAVAILABLE", message: "CMHeadphoneMotionManager requires macOS 14.0 or later", details: nil))
        }
